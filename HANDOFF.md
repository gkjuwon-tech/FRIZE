...
- 2026-06: **🔥 화재 확산 분석 추가** ― 열화상이 '화점 검출'에서 멈추던 걸 '어디로 번지는지'까지 확장. 신규 `frize/recon/fire.hpp`(`FireAnalyzer`, 의존성0): 셀 온도 시계열로 dT/dt(가열률)·확산 전선·확산 방향벡터·재정찰 목표 산출. `frize_mapping`이 이를 `MapPatch`(신규 필드 `fire_front`/`fire_spread`/`revisit`)에 실어 송출. **`frize_scout` 드론 자율탐사 수정**: 미탐사 프런티어만 좇던 걸(+복붙된 중복 블록 제거) `FRIZE_REVISIT_EVERY`회마다 '이미 본' 화재 구역으로 **되돌아가 확산 재확인**하게. 콕핏 `TwinView.qml`에 확산 전선(펄스 마커)·확산 화살표(코멧 트레일)·재정찰 링 렌더. `frize_recon_sim`에 **시간 진행형 화재장(`FireSim`)+재방문 랩** 도입 → g++로 빌드·실행 검증(불 면적 ×9.7 성장, 발화점 2개 분리·방향 +x 검출, 검증 4종 PASS, exit 0). (다음 사람: `FRIZE_FIRE_VOXEL`/`rate_min`/`speed`를 실 열화상으로 튜닝하고, 콕핏에서 확산 화살표 실기 확인하고 도망쳐라.)
- (여기에 네 줄 추가)
